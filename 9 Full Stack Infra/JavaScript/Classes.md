...
- Methods are stored in the prototype, not inside each object  (shared across instances)
- `this` refers to the current instance [[this keyword]]  
- Classes support inheritance using `extends` → [[Inheritance]]
...
Classes cannot be invoked like functions and must be called using `new`.
...
`this` is lost when the method is detached from the object.
...
`bind` ensures the correct `this` context is preserved.
